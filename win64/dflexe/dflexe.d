...
bool doDflSwitch(string arg)
{
	...
	i = cast(int)std.string.indexOf(arg, '=');
	if(i != -1)
	{
	...
	switch(arg)
	{
		...
		case "gui", "winexe", "windowed":
			i = cast(int)std.string.indexOf(optSu, ':');
...
int main(/+ string[] args +/)
{
	...
	if(args.length > 1)
	{
		...
		void doVerCheck(bool vcVerbose = false, bool vcPrintIssues = true)
		{
			...
			ix =  cast(int)std.string.indexOf(x, FINDDFLVER);
			if(-1 == ix)
			{
				bad_readme_ver:
			...
			ix =  cast(int)std.string.indexOf(xver, " by Christopher E. Miller");
			if(-1 == ix)
				goto bad_readme_ver;
			...
			ix =  cast(int)std.string.indexOf(x, FINDTESTEDDMDVER);
			if(-1 == ix)
			{
				//goto bad_readme_ver;
			}
			...
			try
			{
				...
				
				{
					...
					int idx =  cast(int)std.string.indexOf(x, finding);
					if(-1 != idx)
					{
						string sub = x[idx + finding.length .. $];
						idx =  cast(int)std.string.indexOf(sub, '\n');
						if(-1 != idx)
						{
