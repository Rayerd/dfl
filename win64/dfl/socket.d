...
class SocketQueue // docmain
{
	...
	void send(void[] buf)
	{
		if(canwrite)
		{
			...
			if(buf.length > 4096)
				...
				st = cast(int)buf.length;
			
			st = cast(int)sock.send(buf[0 .. st]);
			if(st > 0)
			{
				if(buf.length - st)
				{
	...
	{
		return cast(uint)writebuf.length;
	}
	...
	void readEvent()
	{
		...
		int rd = cast(int)sock.receive(readbuf[rpos .. readbuf.length]);
		if(rd > 0)
			rpos += cast(uint)rd;
	}
